...
class Model:
    def __init__(self, local_rank=-1, arbitrary=True):
        if arbitrary == True:
            self.flownet = IFNet_m()  # default: matches flownet.pkl (7-ch timestep model)
        else:
            self.flownet = IFNet()    # legacy 6-ch model
        self.device()
    ...
    def load_model(self, path, rank=0):
        def convert(param):
            # Handle both DDP-trained (keys have "module." prefix) and non-DDP checkpoints
            has_module_prefix = any("module." in k for k in param.keys())
            if has_module_prefix:
                return {k.replace("module.", ""): v for k, v in param.items() if "module." in k}
            return param  # already clean keys

        if rank <= 0:
            ckpt = torch.load('{}/flownet.pkl'.format(path), map_location='cpu')
            converted = convert(ckpt)
            self.flownet.load_state_dict(converted)
            print(f"[RIFE] Loaded: {type(self.flownet).__name__} ({len(converted)} params)")
    ...
    def inference(self, img0, img1, scale=1, scale_list=None, TTA=False, timestep=0.5):
        ...
        # IFNet_m expects timestep as a tensor concatenated in forward()
        if isinstance(self.flownet, IFNet_m):
